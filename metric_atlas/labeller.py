...
METRICLONGNAME = {'ENGLISH' : {
            ...
            'annualRainyDays' : u'Number of Rainy Days ($>$'+str(cnst.RAINYDAY_THRESHOLD)+'mm day$^{-1}$)',
            'annualRainyDaysPerc' : u'Percentage of Days that are Rainy ($>$'+str(cnst.RAINYDAY_THRESHOLD)+'mm day$^{-1}$)',
    ...
    'FRANCAIS' : {
            ...
            'annualRainyDays' : u'Nombre de journées pluvieuses ($>$'+str(cnst.RAINYDAY_THRESHOLD)+' mm jour$^{-1}$)', #'Number of Rainy Days ($>$'+str(cnst.RAINYDAY_THRESHOLD)+'mm day$^{-1}$)',
            'annualRainyDaysPerc' : u'Pourcentage de journées pluvieuses ($>$'+str(cnst.RAINYDAY_THRESHOLD)+' mm jour$^{-1}$)', #'Percentage of Days that are Rainy ($>$'+str(cnst.RAINYDAY_THRESHOLD)+'mm day$^{-1}$)',
            ...
            'onsetMarteau' : u'Date de déclenchement de la mousson basé sur critères d\'agronomie locale (Marteau)', # 'Local Agronomic Monsoon Onset Date (Marteau)',
...
def getYlab(m, v, anom=None):
    ...
    ylabLUT = {'ENGLISH' : {
            ...
            'annualMaxRain_5dSum': u'5-day Total Precipitation',
            'annualMaxRain_3dSum' : u'3-day Total Precipitation',
            'annualMaxRain_2dSum' : u'2-day Total Precipitation',
            'annualMaxRain_5dMean': u'5-day Mean Precipitation Rate (mm day$^{-1}$)',
            'annualMaxRain_3dMean': u'3-day Mean Precipitation Rate (mm day$^{-1}$)',
            'annualMaxRain_2dMean': u'2-day Mean Precipitation Rate (mm day$^{-1}$)',
            'SPIxMonthly' : u'SPI',
            ...
            'annualHotDaysPerc' : u'Nombre de jours', #when Max Temp >'+str(cnst.HOTDAYS_THRESHOLD)+'$^{\circ}$C',
            'annualRainyDays' : u'Nombre de journée pluvieuse', # (>'+str(cnst.RAINYDAY_THRESHOLD)+'mm day$^{-1}$)',
            'annualRainyDaysPerc' : u'Nombre de journée pluvieuse', # (>'+str(cnst.RAINYDAY_THRESHOLD)+'mm day$^{-1}$)',
            'annualHotDays' : u'Nombre de jours', # when Max Temp >'+str(cnst.HOTDAYS_THRESHOLD)+'$^{\circ}$C',
            'annualExtremeRain30' : u'Nombre de jours', # > 30mm day$^{-1}$',
            'annualExtremeRain50' : u'Nombre de jours', # > 50mm day$^{-1}$',
            'annualExtremeRain100' : u'Nombre de jours', # > 100mm day$^{-1}$',
            'annualStrongWindDays' : u'Nombre de jours', # >'+str(cnst.STRONGWIND_THRESHOLD)+'ms$^{-1}$',
            'wetSpell10': u'Nombre de périodes pluvieuses', # > 10 Days',
            'drySpell6': u'Nombre de périodes sèches', # > 6 Days',
            'annualMaxRain_5dSum': u'Précipitations totales (depuis 5 jours)',
            'annualMaxRain_3dSum' : u'Précipitations totales (depuis 3 jours)',
            'annualMaxRain_2dSum' : u'Précipitations totales (depuis 2 jours)',
            'annualMaxRain_5dMean': u'Précipitations moyennes (depuis 5 jours ; mm jour$^{-1}$)',
            'annualMaxRain_3dMean': u'Précipitations moyennes (depuis 3 jours ; mm jour$^{-1}$)',
            'annualMaxRain_2dMean': u'Précipitations moyennes (depuis 2 jours ; mm jour$^{-1}$)',
            'SPIxMonthly' : u'Indice de précipitations standardisées',
            'SPIbiannual' : u'Indice de précipitations standardisées (bi-annuelle)',
            'onsetMarteau' : u'Jour julien',
            'cdd' : u'Nombre de journées sèches consécutives',
            'pet' : u'Evapotranspiration potentielle (mm jour$^{-1}$)'
            }
            }
