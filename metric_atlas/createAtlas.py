...
def getIntroText(metric):
    ...
    intro_text = {'ENGLISH' : {
            ...
            'SPIxMonthly' : 'The Standardised Precipitation Index (SPI) shown here is defined as the anomaly relative to the baseline period devided by the standard deviation of that baseline period. This metric allows to determine the rarity of drought or periods of anomalously wet events.',
            ...
            'onsetMarteau' : 'Local Agronomic Monsoon Onset Date (Marteau) is defined as the first rainy day (> 1 mm) of two consecutive rainy days (with total precipitation > 20 mm) and no 7-day dry spell (< 5 mm) of rainfall during the subsequent 20 days',
            ...
            'annualRainyDays' : u'Cet indicateur illustre le nombre de jours pour lesquels la pluviométrie est au-dessus du seuil de '+str(cnst.RAINYDAY_THRESHOLD)+u' mm jour$^{-1}$, pendant la période indiquée.', # 'This shows the number of days in the period shown when rainfall was above a threshold of '+str(cnst.RAINYDAY_THRESHOLD)+'mm day$^{-1}$.', could also be: la pluviométrie depasse un seuil de
            'annualRainyDaysPerc' : u'Cet indicateur illustre le pourcentage de jours pour lesquels la pluviométrie est au-dessus du seuil de '+str(cnst.RAINYDAY_THRESHOLD)+u'mm jour$^{-1}$, pendant la période indiquée.',
            'annualHotDays' : u'Cet indicateur illustre le nombre de jours pour lesquels la temperature maximale journalière est au-dessus du seuil de  '+str(cnst.HOTDAYS_THRESHOLD)+lblr.DC+u', pendant la période indiquée.',
            ...
            'annualStrongWindDays' : u'Cet indicateur illustre le nombre de jours pour lesquels la vitesse du vent est au-dessus du seuil de '+str(cnst.STRONGWIND_THRESHOLD)+u'm s$^{-1}$',
...
def getFullCaption(metric, var, region, bc, seas, plotnm, plottype):
    ...
    'allModelRank' : u'Ce diagramme de dispersion illustre xxx_pt_short_xxx xxx_metric_xxx entre xxx_periodstart_xxx et xxx_periodend_xxxxxx_wrt_xxxxxx_seasinfo_xxx. Chaque point indique la moyenne spatiale d\'un modèle sur xxx_region_xxx; les points sont organisés ("rank", sur l\'axe des abscisses) en fonction de la magnitude de leur valeur sur l\'axe des ordonnées. Ce diagramme montre xxx_pt_long_xxx.',# xxx_title_end_xxx.',
            ...
            'allModelBoxplot' : u'Ce tracé en boîte illustre xxx_pt_short_xxx xxx_metric_xxx pour la période entre xxx_periodstart_xxx et xxx_periodend_xxxxxx_wrt_xxxxxx_seasinfo_xxx. Chaque point (ligne rouge horizontale) indique la moyenne spatiale d\'un modèle sur la région xxx_region_xxx, la boîte indiquant l\'intervalle entre le 25ème et le 75ème pourcentile, et les moustaches l\'intervalle entre le 10ème et le 90ème pourcentile. Ce diagramme montre xxx_pt_long_xxx.',# xxx_title_end_xxx.',
            ...
            'allModelHisto' :  u'Cet histogramme illustre xxx_pt_short_xxx xxx_metric_xxx pour la période entre xxx_periodstart_xxx et xxx_periodend_xxxxxx_wrt_xxxxxx_seasinfo_xxx. Chaque barre verticale indique la moyenne spatiale d\'un modèle sur la région xxx_region_xxx. Ce diagramme montre xxx_pt_long_xxx xxx_title_end_xxx.'  ,
            'allModelMonthClim':  u'Ce tracé en boîte de la climatologie mensuelle illustre xxx_pt_short_xxx xxx_metric_xxx pour la période entre xxx_periodstart_xxx et xxx_periodend_xxxxxx_wrt_xxxxxx_seasinfo_xxx. Chaque point (ligne rouge horizontale) indique la moyenne spatiale d\'un modèle sur la région xxx_region_xxx region, la boîte indiquant l\'intervalle entre le 25ème et le 75ème pourcentile, et les moustaches l\'intervalle entre le 10ème et le 90ème pourcentile. Ce diagramme montre xxx_pt_long_xxx.'
    }
    }
    ...
    pt_short = {'ENGLISH' : {
            ...
            'scenarios' : u'l\'écart historique des modèles (pour la période de ' + str(cnst.HIST[0]) + u' à ' +str(cnst.HIST[1])+u'), comparé à tous les scénarios futures disponibles de',
    ...
    if plottype == 'historical':
        ...
        if cnst.LANGUAGE == 'ENGLISH':
            ...
            wrt_txt = u' (par rapport à la période de référence de ' +str(cnst.HIST[0]) + u' à ' + str(cnst.HIST[1])+') '
...
def makeTitlePage(season, texdir):
     ...
     if cnst.LANGUAGE == 'ENGLISH':
         ...
         ifile = os.getcwd() +os.sep + 'metric_atlas'+ os.sep + '0_titlepage_ENGLISH.tex'
         ofile = texdir + os.sep +'0_titlepage_ENGLISH.tex'
         details = u'For '+cnst.ATLAS_REGION[1]+u' in '+ monthLookUp(season) +u' (English version)'
     else:
         ifile = os.getcwd() +os.sep + 'metric_atlas'+ os.sep + '0_titlepage_FRANCAIS.tex'
         ofile = texdir + os.sep +'0_titlepage_FRANCAIS.tex'
         details = u'Pour '+cnst.ATLAS_REGION[1]+u' en '+ monthLookUp(season) +u' (version française)'
...
def runAtlas(season):
    atlas_lang = cnst.METRIC_ATLASDIR + os.sep + cnst.LANGUAGE[:2]

    version = cnst.VERSION
    texdir = atlas_lang + os.sep + season + '_atlas'
    imgdir = cnst.METRIC_PLOTDIR
    if cnst.LANGUAGE == 'ENGLISH':
        coverpage = 'AMMA2050_atlas_coverpage_v'+cnst.VERSION+'_EN.pdf'
    else:
        coverpage = 'AMMA2050_atlas_coverpage_v'+cnst.VERSION+'_FR.pdf'
    if os.path.isdir(texdir):
        shutil.rmtree(texdir, ignore_errors=True)

    if not os.path.isdir(atlas_lang):
        try:
            os.makedirs(cnst.METRIC_ATLASDIR + os.sep + cnst.LANGUAGE[:2])
        except OSError:
            pass
    try:
        os.makedirs(texdir)
    ...
    if cnst.LANGUAGE == 'ENGLISH':
        shutil.copyfile(os.getcwd() + os.sep + 'metric_atlas' + os.sep +  '1introduction_ENGLISH.tex', texdir + os.sep + '1_introduction.tex')
    else:
        shutil.copyfile(os.getcwd() + os.sep + 'metric_atlas' + os.sep + '1introduction_FRANCAIS.tex',
                        texdir + os.sep + '1_introduction.tex')
...
#    last_metric = []
    ...
    for row in cnst.METRICS_TORUN:
        ...
        for var in variable:
            ...
            fmetric.write(
                '\section{' + getMetricNiceName(metric, var).encode('utf-8') + '} \label{sec:' + metric + '}\r\n')
            fmetric.write('\r\n')
...
#                            fmetric.write('\r\n')
    ...
    #                            fmetric.write('\\caption{'+getFullCaption(metric, bc_res, seas, pt)+'}\r\n')
                            ...
                            fmetric.write(
                                '\\caption{' + getFullCaption(metric, var, cnst.ATLAS_REGION[1], bc_res, seas, pn,
                                                              pt).encode('utf-8') + '}\r\n')
...
#            print(section_counter)
    ...
    # print(plot_sections)
    with open(texdir + "/atlas_template.tex", "r") as fin, open(texdir + os.sep + "atlas_" + cnst.ATLAS_REGION[
        0] + '_' + seas + '_v' + version +'_' + cnst.LANGUAGE[:2] + ".tex", "w") as fout:
           for line in fin:
...
#                    print('\input{'+pltsec+'}')
    ...
    subprocess.call(["pdflatex", "-output-directory", texdir, "-interaction", "batchmode", texdir + os.sep + "atlas_"+cnst.ATLAS_REGION[0] +'_' + seas + '_v' +version+'_' + cnst.LANGUAGE[:2]+".tex"])
    subprocess.call(["pdflatex", "-output-directory", texdir, "-interaction", "batchmode", texdir + os.sep + "atlas_"+cnst.ATLAS_REGION[0] +'_' + seas + '_v' +version+ '_' + cnst.LANGUAGE[:2]+".tex"])
   # pdb.set_trace()

    afile =  "atlas_" + cnst.ATLAS_REGION[0] + '_' + seas + '_v' + version +'_' + cnst.LANGUAGE[:2] + ".pdf"
    apath = texdir + os.sep + afile
    if os.path.isfile(apath):
        print('File successfully created: ' + apath)
        shutil.move(apath, atlas_lang + os.sep + afile)
    else:
