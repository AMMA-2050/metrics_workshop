...
def master(variable,scenario,bc_and_resolution,inpath, outpath, season,region,calc_file,xmin,xmax,ymin,ymax,plotter,overwrite): # All these variables are passed through from ultimo_burrito
 ...
 for sc in scenario:
  for bc in bc_and_resolution:   
   for seas in season:   
    ...
    files_good, modelID = load_file_names.load_file_names(inpath,variable,sc,bc) # load all suitable files names and return a list with file names and another list with just 
                                                                                               # the model IDs 
...
# identifier that is easy to understand for netCDF files and many other wonderful things (calorie free chocolate maybe?)
    ...
    for fle in files_good: # loop through files found in the load_file_names 
           for nme in modelID: # seperate loop here to set up the correct ncfile name
               ...
               if nme in fle:
                  nc_file = outpath+'/'+str(calc_file)+'_'+str(bc)+'_'+str(sc)+'_'+str(seas)+'_'+str(region)+'_'+str(nme)+'.nc'
