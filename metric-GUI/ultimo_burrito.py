...
plot_dictionary =     {'line': 'line_plot',
		'line plot year':'line_plot_year',
                'scatter':'scatter',
                'scatter year':'scatter_year',
                'scatter_month':'scatter_month',
                '2dc': 'plot_2d_contour',
                'Contourf with map': 'contourf_map',
                'No map contourf': 'contourf_no_map'
                }
...
def runmaster(variable,scenario,bc_and_resolution, inpath,outpath, season,region,calc_file,xmin,xmax,ymin,ymax,plotter,overwrite):
   import master
...
#   print 'Please type Y to continue'
   ...
   if continu == 'Y' or continu == 'y':
      master.master(variable,scenario,bc_and_resolution, inpath,outpath, season,region,calc_file,xmin,xmax,ymin,ymax,plotter,overwrite)
...
class mainWindow(object): # everything in here will appear in the main window of the GUI (what appears when you hit run)
...
# just throw things on the left or right of the screen and ask Tkinter to sort it out.
        ...
        runbutton = Button(buttons,text='Run code', command = lambda: runmaster(variable,period,bc,inpath, outpath, season,area_name,file_name,xmin,xmax,ymin,ymax,plotter,overwrite))
...
# Lets do these two here.
        ...
        lbbound = Listbox(lists, listvariable=items, yscrollcommand=scrollbar.set, width=15, height=5, exportselection=False)
...
#        lbvar.pack(side="left",expand=True)
        ...
        global plot_dictionary # we are going to use the keys from plot_dictionary, so go and grab this

        # so the first thing we do is get the directory keys as a set of string varaibles to put in the listbox
        # for ultimo_burrito, we have sorted the keys alphabetically as well, but clean.
        # 
        items = StringVar(value=tuple(sorted(plot_dictionary.keys()))) 
        # next, we create the listbox instance.
        # The only necessary part of this line is:
        # lbplot = Listbox(root)
        #
        # But, as we have a set of keys to put in to the listbox, we really
        # might as well throw them in here.
        # While we are at it, we set a width and height for the box so that all listboxes are uniform (because it is clean)
        lbplot = Listbox(lists, listvariable=items, width=15, height=5)
        #
        # So when do you want the listbox to do something, and what do you want it to do?
        # Here we bind an mouse event to a command.
        # The simple translation of the below line is - "When you double left click on something in list box, do whatever OnDoublePlot tells you to do.
        #
        # For single left clicks, change this to <Button-1>
        # There are other options as well. Just look online.
        lbplot.bind("<Double-Button-1>", OnDoublePlot)
        # Pack, for you must always pack. Can't remember why we do expand = True, but you should (I guess)
