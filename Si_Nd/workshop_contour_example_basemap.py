...
def workshop_contour_example_basemap():
    ...
    mydata = gent('/nfs/see-fs-01_teaching/earv053/metrics_workshop/example_code/TRMM_v7_Marteau_onsets_1998_dry_spell_30.csv',delimiter=',')
    ...
    print(len(ltd), len(lat))
    #levs = [119, 129, 139, 149, 159, 169, 179, 189, 200, 210, 220, 230, 240]
    #print(len(levs))
    a = np.max(mydata[:,:])
    b = np.min(mydata[:,:])
    
    levs = np.linspace(np.amin(mydata[:,:]), a, 13)
    print(levs)
    # Open figure window
